...
<div className="font-sans flex" style={{ width: "794px", height: "1123px" }}>
      {/* Left Sidebar */}
      <div className="w-1/3  bg-emerald-800 text-white p-6 rounded-l-lg">
        {formData?.personalInfo && (
          <div className="mb-8">
            {formData.personalInfo.name && (
              <h1 className="text-xl font-bold mb-2 border-b border-emerald-600 pb-2">{formData.personalInfo.name}</h1>
            )}
            {formData.personalInfo.position && (
              <p className="text-emerald-300 font-[500] text-xs uppercase mb-4">{formData.personalInfo.position}</p>
            )}
        ...
        {formData.skills && formData.skills.length > 0 && (
          <section className="mb-8">
            <h2 className="text-lg font-bold mb-4 border-b border-emerald-600 pb-2">SKILLS</h2>
            <div className="flex flex-wrap gap-2">
              {formData.skills.map((skill, index) => (
                <span key={index} className="bg-emerald-700 rounded-full px-3 py-1 text-[14px] font-bold inline-block">
                  {skill}
                </span>
              ))}
            </div>
        ...
        {formData.education && (
          <section className="mb-8">
            <h2 className="text-lg font-bold mb-4 border-b border-emerald-600 pb-2">EDUCATION</h2>

            {formData.education?.higher && (
              <div className="mb-6">
                <h3 className="font-semibold text-[0.7rem]">{formData.education.higher.name}</h3>
                <p className="italic text-[0.7rem] text-emerald-300">{formData.education.higher.degree}</p>
                <p className="text-[0.7rem] text-emerald-500 mb-2">
            ...
            {formData.education?.twelfth && (
              <div className="mb-6">
                ...
                <p className="italic text-[0.7rem] text-emerald-300">Senior Secondary (XII)</p>
                <p className="text-[0.7rem] text-emerald-500 mb-2">
            ...
            {formData.education?.tenth && (
              <div className="mb-6">
                ...
                <p className="italic text-[0.7rem] text-emerald-300">Secondary (X)</p>
                <p className="text-[0.7rem] text-emerald-500 mb-2">
      ...
      <div className="w-2/3 bg-white p-6 rounded-r-lg">
        ...
        {Array.isArray(formData.experience) && formData.experience.length > 0 && (
          <section className="mb-8">
            <h2 className="text-lg font-bold mb-4 text-gray-800 relative pl-4 before:content-[''] before:absolute before:left-0 before:top-0 before:bottom-0 before:w-1 before:bg-emerald-800">
              EXPERIENCE
            </h2>
            {formData.experience.map((exp, index) => (
              ...
              >
                <div className="absolute left-0 top-0 w-2 h-2 rounded-full bg-emerald-800 -ml-[3px]"></div>
