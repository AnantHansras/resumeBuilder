        ...
        {formData?.personalInfo && (
          <div className="mb-8">
            {formData.personalInfo.name && (
              <h1 className="text-xl font-bold mb-2 border-b border-gray-600 pb-2">
                {formData.personalInfo.name}
              </h1>
            )}
            {formData.personalInfo.position && (
              <p className="text-gray-300 font-[500] text-xs uppercase mb-4">
                {formData.personalInfo.position}
              </p>
            )}
          ...
          <section className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {formData.skills.map((skill, index) => (
                <span
                  key={index}
                  className="bg-gray-700 rounded-full px-3 py-1 text-[14px] font-bold inline-block"
                >
                  {skill}
                </span>
              ))}
        ...
        {Array.isArray(formData.experience) && formData.experience.length > 0 && (
          <section className="mb-8">
            ...
            {formData.experience.map((exp, index) => (
              <div key={index} className="mb-6 relative pl-6 before:content-[''] before:absolute before:left-0 before:top-0 before:bottom-0 before:w-[2px] before:bg-gray-200">
                ...
                <p className="text-[0.65rem] font-sans text-gray-600 mb-2">
                  {exp.duration}
                </p>
            ...
            formData.projects.map((proj, index) => (
              <div key={index} className="mb-6 relative pl-6 before:content-[''] before:absolute before:left-0 before:top-0 before:bottom-0 before:w-[2px] before:bg-gray-200">
                ...
                <div className="flex justify-between items-baseline">
                  <h3 className="font-semibold font-sans text-[0.75rem] text-gray-800">
                    {proj.name}
                  </h3>
                  {proj.link && (
                    <a
          ...
          <section className="mb-4">
            ...
            <ul className="space-y-2 pl-6">
              {formData.achievements.map((a, index) => (
                <li className="text-[0.7rem] font-sans relative pl-4 before:content-[''] before:absolute before:left-0 before:top-[0.5em] before:w-2 before:h-2 before:bg-gray-800 before:rounded-full" key={index}>
                  {a}
                </li>
              ))}
            </ul>
